...
public class TraderSellStrategy1Tactic extends GenericTactic {
...
// ---- CONSTRUCTORS
    ...
    //Default constructor.
	TraderSellStrategy1Tactic() {}
	
	//Used constructor
	TraderSellStrategy1Tactic(double multOfferVol, double sbd, double multMustSellPrice, double multRestPrice, int learnID) {
...
// ---- UPDATE THE TACTIC SELL OFFERS	
	...
	public void updatetacticselloffers() {
		...
		offerMustSellVol = createMustSellVolOffer(expectedprice,physicalnetposition,10);		//The 10, 10 is not used further, but just cause of laziness in construction an new method.
		double mustsellvol = offerMustSellVol.getCertVolume();
		offerRestVol = createRestVolOffer(expectedprice,physicalnetposition, 1000, mustsellvol);
		...
		if (tacticselloffers.size()<1) {
			throw new IllegalArgumentException("DEBUG: Should not have only one or zero sell offer");
		}
	}
...
//		maxppvolume = this.getmyStrategy().getmyAgent().getagentcompanyanalysisagent().getvolumeanalysisagent().getvolumeprognosis().getCurObPdCertProduction(); //The max pp volume is equal to the expected production of the twelve next ticks. This value itself is produced in the volumeanalysis agent.
	...
	//The two offers are very simple after reeningeering the original selloffertactic (20150630) KK
...
// ---- UTILITY CALCULATIONS
	...
	public void calcUtilityForCurrentTick() {
		...
		// Calculate the utility for the current tick, using the utility function for the agent
		 ...
		 retUtility = myStrategy.myAgent.getutilitymethod_additional().CalcUtilityWithHistory(ShortTermMarket.getcurrentmarketprice(), tacticselloffers, ShortTermMarket.getshareofmarignaloffersold());
