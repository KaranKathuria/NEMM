...
public class PAUtilityMethod extends GenericUtilityMethod{
	...
	public PAUtilityMethod(int functionFlag) {
		...
		//stream1Uniform = RandomHelper.createUniform(); KK 20160111 Removed as we want to controll randomness with only using one stream.
...
//				curOfferShareCleared = s.get(i).getShareCleared();
				...
				if (curOfferVol > 0) {
					...
					if (i==0) {
						...
						if (AllVariables.limitproducerCV) {
							certValue = Math.min(certValue, myAgent.getagentcompanyanalysisagent().getmarketanalysisagent().getCertValueDeveloper());
						}	
